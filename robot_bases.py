...
class Joint:
  ...
  def get_reaction_force(self):
    _, _, reaction_force, _ = self._p.getJointState(self.bodies[self.bodyIndex], self.jointIndex)
    return reaction_force

  def get_motor_torque(self):
    _, _, _, motor_torque = self._p.getJointState(self.bodies[self.bodyIndex], self.jointIndex)
    return motor_torque

  def enable_sensor(self):
    self._p.enableJointForceTorqueSensor(self.bodies[self.bodyIndex], self.jointIndex, enableSensor=1)

  def disable_sensor(self):
    self._p.enableJointForceTorqueSensor(self.bodies[self.bodyIndex], self.jointIndex, enableSensor=0)
