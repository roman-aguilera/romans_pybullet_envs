...
class OctopusEnv(gym.Env):
  ...
  def reset(self):
    if (self.physicsClientId < 0): #if it is the first time we are loading the simulations 
      ...
      self._p.setGravity(gravX=0, gravY=0, gravZ=-9.8, physicsClientId=self.physicsClientId)
    ...
    for i in range(self.number_of_joints_urdf):
      ...
      self._p.resetJointState(bodyUniqueId = self.octopusBodyUniqueId, physicsClientId=self.physicsClientId , jointIndex=i, targetValue=random.uniform(-np.pi/2, np.pi/2), targetVelocity=random.uniform(-np.pi, np.pi) ) #tagetValue is angular (or xyz)  position #targetvelocity is angular (or xyz) veloity
    self.time_stamp=0 
  ...
  def step(self, actions):
    ...
    #set torques
    self.torques = 0*actions #1000*actions
